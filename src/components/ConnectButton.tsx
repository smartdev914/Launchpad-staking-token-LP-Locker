...
const ConnectButton: FC<ConnectButtonProps> = ({ color = 'light', className = '', style = {} }) => {
  ...
  const onClickConnect = useCallback(() => {
    if (account) {
      ...
      window.localStorage.setItem('STEALTHPAD_AUTO_CONNECT', '0')
    } else {
      // not connected
      mounted(connect())
        .then(() => window.localStorage.setItem('STEALTHPAD_AUTO_CONNECT', '1'))
        .catch((err) => {
          console.error(err)
  ...
  useMount(() => {
    if (window.localStorage.getItem('STEALTHPAD_AUTO_CONNECT') === '1') {
