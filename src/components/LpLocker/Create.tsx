...
const Create: React.FC = () => {
  ...
  const { address: lockerManagerAddress, lpLockerFee, tokenLockerFee, createTokenLocker, createLpLocker } = useTokenLockerManagerV1Contract()
  ...
  const [lockDate, onDateChange] = useState(new Date());
  ...
  useEffect(() => {
    if (tokenAddress) {
      onInputAddress(tokenAddress)
    }
  }, [mounted, getTokenData, isLpToken, tokenAddress])
  ...
  const onInputAddress = useCallback(
    (address: string) => {
      ...
      if (!address || !isLpToken || !getTokenData) {
  ...
  const onClickSubmit = useCallback(() => {
    ...
    if (approved) {
      ...
      if (lpToken) {
        mounted(createLpLocker(tokenData.address, utils.parseUnits(amount, tokenData.decimals), (unlockTime - Math.floor(Date.now()/1000)), lpLockerFee))
          .then((id: number) => {
            setCanSubmit(true)
            setIsSubmitting(false)
            navigate(`/lp-locker/${getNetworkDataByChainId(chainId ?? 0)?.urlName || chainId}/${id}`)
          })
          .catch((err) => {
            console.error(err)
            setCanSubmit(true)
        ...
        mounted(createTokenLocker(tokenData.address, utils.parseUnits(amount, tokenData.decimals), (unlockTime - Math.floor(Date.now()/1000)), tokenLockerFee))
          .then((id: number) => {
            setCanSubmit(true)
            setIsSubmitting(false)
            navigate(`/lp-locker/${getNetworkDataByChainId(chainId ?? 0)?.urlName || chainId}/${id}`)
            ...
            <FormOuter>
              <div className="flex justify-between items-center">
                ...
                <AddressInput
                  ...
                  value={tokenAddress}
                  onInput={(event) => onInputAddress(event.currentTarget.value)}
                /> : 
                <AddressInput
                  className="flex-grow"
                  placeholder="Enter liquidity pair or token address"
