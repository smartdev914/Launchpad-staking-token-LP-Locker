...
const StakingComponent: React.FC<StakingProps> = ({ viewMode }) => {
  ...
  const setupAllStaking = useCallback(() => {
    ...
    if (idToUse) {
      ...
      // getTokenLockersForAddress(accountToCheck).then((i  ds: Array<number>) =>
  ...
  return (
    <Outer>
      <div className="p-2 flex justify-center items-center">
        <Link to="/staking">
          <Button
            className={`rounded-l-none rounded-r-none text-xl min-w-[120px] ${
              viewMode === 'split'
                ? ' border-2 text-blue-500 border-blue-500 rounded-t-md'
