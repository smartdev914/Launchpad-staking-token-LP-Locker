...
const Staking: React.FC<StakingProps> = ({
  ...
  const [unlockTimeForAccount, setUnlockTimeForAccount] = useState<number>(0)
  const [autoClaimForAccount, setAutoClaimForAccount] = useState<boolean>(true)
  ...
  const updateStakingDataForAccount = useCallback(() => {
    ...
    mounted<BigNumber>(stakingContract.getUnlockTime(account))
      .then((result) => {
        setUnlockTimeForAccount(BigNumber.from(result).toNumber())
      })
      .catch((err: Error) => {
        console.error(err)
        setUnlockTimeForAccount(0)
      })

    mounted<boolean>(stakingContract.getAutoClaimOptOut(account))
      .then((result: boolean) => {
        setAutoClaimForAccount(result)
      })
      .catch((err: Error) => {
        console.error(err)
        setAutoClaimForAccount(false)
      })
  ...
  const toggleAutoClaimOptOut = () => {
    if (!stakingContract) return
    
    mounted(stakingContract.setAutoClaimOptOut(!autoClaimForAccount))
      .then((result) => {
        setAutoClaimForAccount(!autoClaimForAccount)
      })
      .catch((error) => {
        console.log(error)
      })
          ...
          <>
            <div className="flex-grow flex flex-col gap-3">
              ...
              <div className="flex justify-between gap-2 w-full items-center">
                {unlockTimeForAccount > 0 && (
                  <div className='w-full mt-4 text-[18px] bg-white dark:bg-gray-900 rounded h-11 items-center justify-center flex'>
                    Lock Time: {new Date(unlockTimeForAccount*1000).toLocaleString()}
                  </div>
                )}
              ...
              {stakingContract && (
                ...
                >
                  ...
                  <PrimaryButton
                    disabled={!account}
                    onClick={toggleAutoClaimOptOut}
                  >
                    {autoClaimForAccount && 'Disable auto claim'}
                    {!autoClaimForAccount && 'Enable auto claim'}
                  </PrimaryButton>
